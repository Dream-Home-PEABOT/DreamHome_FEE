...
describe("App", () => {
    ...
    questionResults = getQuestions.mockResolvedValue({
      annual_salary: {
        attributes: {
          ...
          description:
            "Gross income is the total amount you earn (typically over the course of a year) before expenses. Net income is the profit your business earns after expenses ",
          information:
          ...
          note:
            "The amount of money you earn plays a smaller role in getting a mortgage than you might think. ",
  ...
  it("User should see error message if question is unanswered and next is pressed", async () => {
    ...
    expect(
      screen.getByText(/Sorry but we need this information/)
    ).toBeInTheDocument();
  ...
  it("User should be able to fill out full questionare by clicking next", async () => {
    ...
    expect(
      screen.getByText(/What is your current credit score?/)
    ).toBeInTheDocument();
